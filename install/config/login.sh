...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the autarky file
    echo "splash" | sudo tee -a /etc/cmdline.d/autarky.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the autarky file
    echo "quiet" | sudo tee -a /etc/cmdline.d/autarky.conf
  fi
...
if [ "$(plymouth-set-default-theme)" != "autarky" ]; then
  sudo cp -r "$HOME/.local/share/autarky/default/plymouth" /usr/share/plymouth/themes/autarky/
  sudo plymouth-set-default-theme -R autarky
fi
...
if [ ! -f /etc/systemd/system/autarky-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/autarky-seamless-login.service
[Unit]
Description=Autarky Seamless Auto-Login
Documentation=https://github.com/basecamp/autarky
...
# Enable autarky-seamless-login.service only if not already enabled
if ! systemctl is-enabled autarky-seamless-login.service | grep -q enabled; then
  sudo systemctl enable autarky-seamless-login.service
fi
